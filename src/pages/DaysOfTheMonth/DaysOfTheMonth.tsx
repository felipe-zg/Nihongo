...
type DaysOfTheMonthFieldPaths = 
  | ['id']
  | ['day']
  | ['reading']

const emptyDays: DayOfTheMonth = {
  id: 1,
  day: "",
  reading: "",
};
...
type Props = {
  deckItems: Array<DayOfTheMonth>;
};

const DaysOfTheMonth: React.FC<Props> = ({ deckItems }) => {
  const [currentDeckPosition, setCurrentDeckPosition] = React.useState(0);
  const [isFlipped, setIsFlipped] = React.useState(false);
  const [ answer, setAnswer ] = React.useState<DayOfTheMonth>(emptyDays);
  ...
  const updateAnswerField = (fieldPath: DaysOfTheMonthFieldPaths, value: string) => {
    setAnswer((prev) => {
  ...
  return (
    <div>
      <Center>
        ...
        <br/><br/><br/>
        <Heading size="4xl">
          <Text color="emerald.500">{currentItem.day}</Text>
        </Heading>
        <br/>
