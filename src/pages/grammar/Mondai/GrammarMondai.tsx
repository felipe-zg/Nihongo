...
function BoxHeader({ title, category, refersTo }: { title: string, category: string, refersTo?: number }) {
  return (
    <Box position="relative" w="100%">
      <HStack mb={1} alignItems="center">
        <Text fontSize="lg" color="primary.500" flex={1} textAlign="left">
          {category}
        </Text>

        <Text fontSize="lg" color="pink.500" flex={1} textAlign="center">
          {title}
        </Text>
  ...
  return (
    <Box padding={5} minHeight={"100vh"}>
      {items.map((item) => {
        ...
        let color = 'black';
        return (
          <Box key={`${item.category}${item.id}`} marginBottom={8} px={5} pb={5} pt={3} backgroundColor="#fffff0" borderRadius={10} shadow={4} >
            <BoxHeader title={item.type} category={item.category} refersTo={item.refersTo} />
            <VStack space={4}>
